...
void HuffmanEncodingTree::GenerateFromFrequencyTable( const unsigned int frequencyTable[ 256 ] )
{
	int counter;
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
  ...
  RakNet::BitStream bitStream;
    ...
    const HuffmanEncodingTreeNode* currentNode = leafList[counter];
		...
		encodingTable[ counter ].bitLength = static_cast<unsigned char>(
        bitStream.CopyData(&encodingTable[counter].encoding));
...
void HuffmanEncodingTree::EncodeArray(const unsigned char *input,
                                      const size_t sizeInBytes, RakNet::BitStream * output )
{		
	unsigned counter;
    ...
    const auto remainingBits = static_cast<unsigned char>(8 - (output->GetNumberOfBitsUsed() % 8));
...
unsigned HuffmanEncodingTree::DecodeArray( RakNet::BitStream * input,
                                          const BitSize_t sizeInBits,
                                          const size_t maxCharsToWrite, unsigned char *output )
{

  unsigned outputWriteIndex = 0;
  const HuffmanEncodingTreeNode* currentNode = root;
...
void HuffmanEncodingTree::DecodeArray( unsigned char *input,
                                      const BitSize_t sizeInBits, RakNet::BitStream * output )
{
