...
void DataCompressor::Compress( unsigned char *userData,
                              const unsigned sizeInBytes, RakNet::BitStream * output )
{
  ...
  const BitSize_t writeOffset1 = output->GetWriteOffset();
	output->Write(static_cast<unsigned int>(0));	// Dummy value
  const BitSize_t bitsUsed1 = output->GetNumberOfBitsUsed();
	tree.EncodeArray(userData, sizeInBytes, output);
  const BitSize_t bitsUsed2 = output->GetNumberOfBitsUsed();
  const BitSize_t writeOffset2 = output->GetWriteOffset();
...
unsigned DataCompressor::DecompressAndAllocate( RakNet::BitStream * input, unsigned char **output )
{
  ...
  input->ReadCompressed(destinationSizeInBytes);
	...
	*output = static_cast<unsigned char*>(
      rakMalloc_Ex(destinationSizeInBytes, _FILE_AND_LINE_));
	tree.GenerateFromFrequencyTable(frequencyTable);
  const unsigned int decompressedBytes =
